    ...
    try:
        async with sse_client(url) as (read, write):
            async with ClientSession(read, write) as session:
                ...
                add_feed_result = await session.call_tool("add_feed", {
                ...
                event_auto = await session.call_tool("add_event", {
                    "name": "Auto Event",
                ...
                if match:
                    ...
                    list_res = await session.call_tool("list_events", {{"namespace": auto_namespace}})
                ...
                event_client = await session.call_tool("add_event", {
                    "name": "Client Event",
                ...
                # Verify listing
                list_client = await session.call_tool("list_events", {{"namespace": client_ns}})
                ...
                if match:
                    list_iso = await session.call_tool("list_events", {{"namespace": auto_namespace}})
