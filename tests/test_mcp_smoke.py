...
@pytest.mark.asyncio
async def test_list_feeds(mcp_session):
    res = await mcp_session.call_tool("list_feeds", {"userspace": USERSPACE})
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_search_articles(mcp_session):
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_get_recent_articles(mcp_session):
    res = await mcp_session.call_tool(
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_search_events(mcp_session):
    res = await mcp_session.call_tool(
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_search_trends(mcp_session):
    res = await mcp_session.call_tool(
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_user_lifecycle(mcp_session):
    # List users (should succeed even if empty)
    res = await mcp_session.call_tool("list_users", {})
    assert res.content[0].text is not None
    ...
    if user_id:
        ...
        # Delete user (cleanup)
        res = await mcp_session.call_tool("delete_user", {"user_id": user_id})
        assert res.content[0].text is not None
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_list_unannotated_articles(mcp_session):
    res = await mcp_session.call_tool(
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_get_annotation_stats(mcp_session):
    res = await mcp_session.call_tool(
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_agent_config_lifecycle(mcp_session):
    # Add config
    res = await mcp_session.call_tool(
        "add_agent_config",
        {
    ...
    try:
        result = json.loads(result_text) if not isinstance(res.content[0].text, dict) else res.content[0].text
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_staleness_tools(mcp_session):
    import re
