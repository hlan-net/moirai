...
def build_article_selector(since=None, feed_id=None, event_id=None, trend_id=None):
    ...
    if trend_id:
        ...
        if trend:
            ...
            if event_ids:
                ...
                events_in_trend = query_couchdb("events", selector={"_id": {MONGO_IN: event_ids}}, fields=["article_links"])
                for event_doc in events_in_trend:
                    article_ids_from_filters.extend(event_doc.get("article_links", []))
            else:
                return {"_id": {"$eq": "no_match"}} # No articles if trend has no events
        else:
    ...
    if event_id:
        ...
        if event:
            ...
            if trend_id: # If trend_id also exists, intersect with trend's article_ids
                article_ids_from_filters = list(set(article_ids_from_filters) & set(event_article_links))
            else: # Otherwise, use event's article_ids
                article_ids_from_filters.extend(event_article_links)
            
            if not article_ids_from_filters and (trend_id or event_id): # If a filter was applied and resulted in no matches
    ...
    if article_ids_from_filters:
        ...
        selector["_id"] = {MONGO_IN: list(set(article_ids_from_filters))} # Use set to remove duplicates
    ...
    if feed_id:
        if "_id" in selector: # If event/trend filter already applied
            ...
            selector["_id"][MONGO_IN] = list(set(selector["_id"][MONGO_IN]) & set(feed_article_ids))
