...
from api.validation import AgentStatus, AgentTriggerType, AgentTargetDB
...
class AgentOrchestrator(threading.Thread):
    ...
    def check_and_run_agents(self):
        active_agents = query_couchdb(self.agent_configs_db, selector={"status": AgentStatus.ACTIVE.value})
        ...
        if any(a.get("trigger_type") == AgentTriggerType.ON_NEW_ARTICLE.value for a in active_agents):
            new_articles = query_couchdb(
                self.articles_db,
                selector={"published": {"$gt": self.last_article_check_time.isoformat()}},
                sort=[{"published": "asc"}]
            )
            if new_articles:
        ...
        for agent_config in active_agents:
            ...
            if agent_config.get("trigger_type") == AgentTriggerType.SCHEDULED.value and "last_run_at" not in agent_config:
                agent_config["last_run_at"] = (datetime.now(timezone.utc) - timedelta(days=365)).isoformat() # Initialize to a year ago
                update_couchdb_doc(self.agent_configs_db, agent_config["_id"], agent_config) # Persist update

            try:
                self.run_agent_if_due(agent_config, new_articles)
            except Exception as e:
                logger.error(f"Error running agent {agent_config.get('_id', 'N/A')}: {e}")
    ...
    def run_agent_if_due(self, agent_config: dict, new_articles: list[dict]):
        ...
        if trigger_type == AgentTriggerType.ON_NEW_ARTICLE.value:
            if new_articles:
                logger.info(f"Triggering on_new_article agent {agent_id} for new articles.")
                self.execute_agent_logic(agent_config, new_articles=new_articles, llm_config=agent_config.get("llm_model_config"))
            ...
            if self.is_scheduled_agent_due(last_run_at_str, schedule_interval):
                ...
                self.execute_agent_logic(agent_config, llm_config=agent_config.get("llm_model_config"))

    def is_scheduled_agent_due(self, last_run_at_str: str, schedule_interval: str) -> bool:
        if not schedule_interval:
            return False

        last_run_at = datetime.fromisoformat(last_run_at_str) if last_run_at_str else datetime.min.replace(tzinfo=timezone.utc)
        ...
        if interval_unit == "s":
            ...
            logger.warning(f"Unknown schedule interval unit: {schedule_interval}. Agent cannot be run.")
            return False
    ...
    def execute_agent_logic(self, agent_config: dict, **kwargs):
        ...
        if not logic_module_path:
            logger.error(f"Agent {agent_config.get('_id')} has no logic_module specified.")
            return
        ...
        try:
            ...
            logic_function(agent_config=agent_config, mcp_client=self.mcp_client, **kwargs)
            ...
            logger.error(f"Failed to execute logic for agent {agent_config.get('_id')} from {logic_module_path}: {e}")
